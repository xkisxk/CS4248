...
class MultilingualDatasetManagerWithEval(object):
    def __init__(self, args, lang_pairs, langs, dicts, sampling_method):
        ...
        self.extra_lang_pairs = (
            list({p for _, v in args.extra_lang_pairs.items() for p in v.split(",")}) if args.extra_lang_pairs else []
        )
    ...
    def setup_data_manager(cls, args, lang_pairs, langs, dicts, sampling_method):
        return MultilingualDatasetManagerWithEval(args, lang_pairs, langs, dicts, sampling_method)
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            ...
            metavar="SRCTGT",
            help="prepend to the beginning of source sentence the source or target " "language token. (src/tgt)",
        )
        parser.add_argument(
            "--decoder-langtok",
            action="store_true",
    ...
    def prepare(cls, load_dictionary, args, **kargs):
        ...
        def check_langs(langs, pairs):
            ...
            for src, tgt in pairs:
                if src not in langs or tgt not in langs:
                    messages.append(
                        f"language pair {src}-{tgt} contains languages " "that are not in the language dictionary"
                    )
            if len(messages) > 0:
        ...
        # Temporary fix
        # language_list = ["zh", "zh1", "zh2", "en"]
        check_langs(
            language_list,
            ([p.split("-") for p in args.lang_pairs] if training else [(args.source_lang, args.target_lang)]),
        )
    ...
    def get_data_paths_and_lang_pairs(self, split):
        ...
        if split == getattr(self.args, "valid_subset", None):
            # print("NISHANT -- ", self.eval_lang_pairs)
                ...
                param_list.append(
                    {
                        ...
                        "split": split,
                        "src": src,
                        "src_dict": self.get_source_dictionary(src) if src and data_category != "mono_dae" else None,
                        "tgt": tgt,
    ...
    def get_sampling_ratios(self, data_param_list, datasets, epoch, shard_epoch=None):
        if self.args.sampling_weights_from_file:
            ...
            print("Sampling Weights:", self.args.sampling_weights)
        ...
        if sample_ratios is not None:
            logger.info(
                "| Upsample ratios: {}".format(list(zip(map(lambda x: x["key"], data_param_list), sample_ratios)))
            )
